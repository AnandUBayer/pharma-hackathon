  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="lmnt-theme-on-surface text-2xl font-bold">Day Planner</h2>
          <p className="lmnt-theme-on-surface opacity-70 text-sm">{formatDate(selectedDate)}</p>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="lmnt-theme-secondary-bg lmnt-theme-on-secondary rounded-full p-3 shadow-lg hover:shadow-xl transition-shadow"
        >
          <Plus size={24} />
        </button>
      </div>
